...
function createEmailBody(name: string, companyName: string,finalPOC1Name: string , finalPOC1Phone: string , finalPOC2Name: string , finalPOC2Phone: string) {
  ...
  return `
    ...
    <ul>
    <li>Securing positions in top-tier companies like <strong>Google, Zomato, Swiggy, Amazon, Atlassian, Adobe, Walmart, NVIDIA, Flipkart, Cisco, NASDAQ, MPL, AngleOne, OnePlus, Intuit, Productiv, Zeotap, Commvault, American Express, VMware, Tekion Corp, Infoedge, Rakuten</strong>, and more.</li>
...
export async function POST(req: Request) {
  try {
    ...
    const finalPOC1Name = (poc1Name?.toString().trim()) || "Abhinav Om";
    const finalPOC1Phone = (poc1Phone?.toString().trim()) || "+91 9508940354";
